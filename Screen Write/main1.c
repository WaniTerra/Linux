...
#define MAX_PARTICLES 100 // Patlama parçacığı sınırı
...
Target target;
Particle particles[MAX_PARTICLES]; // Parçacık Havuzu
...
void update(float dt) {
    ...
    if (keystate[SDL_SCANCODE_D]) player.x += player.speed * dt;
...
int main(int argc, char* argv[]) {
    ...
    window = SDL_CreateWindow("Savunma Sanayi Simülasyonu", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, WIDTH, HEIGHT, 0);
    ...
    while (is_running) {
        while (SDL_PollEvent(&event)) {
            ...
            if (event.type == SDL_KEYDOWN) {
                if (event.key.keysym.sym == SDLK_SPACE) shoot_bullet();
            }
        }

        int time_to_wait = 16 - (SDL_GetTicks() - last_frame_time);
        if (time_to_wait > 0 && time_to_wait <= 16) SDL_Delay(time_to_wait);
